let documents = [
  {
    title: "Care este diferența dintre specializările domeniului Matematică?",
    text: `Domeniul Matematică se împarte în 3 specializări:
  ...
  {
    title: "Daca mă duc la mate-info pot să iau și licența de la info?",
    text: `**Da.** Dupa finalizarea cursurilor de licență în domeniul Matematică,
  ...
  {
    title: "Sportul este obligatoriu?",
    text: `Doar pentru anul I.`,
  },
  {
    title:
      "Dacă am intrat la taxă în primul an este vreo posibilitate de a trece la buget ulterior?",
    text: `**Da.** Înaintea începerii fiecărui an universitar se va realiza o reclasificare
      unde se va ține cont de rezultatele obținute în anul universitar precedent.`,
  },
  {
    title: "Dacă fac mate-info, pot să fac toată materia de la info?",
    text: `**Da.** Cât timp ești student la specializarea Matematică–Informatică,
  ...
  {
    title: "De ce să aleg București când am Cluj și Iași?",
    text: `Diferențele nu sunt mari între facultățile din aceste orase. În mare
  ...
  {
    title:
      "Mă pot muta de la un domeniu la altul? De exemplu, de la CTI la Informatică?",
    text: `**Nu.** Pentru a te transfera la alt domeniu, trebuie să dai din nou admiterea
      și să intri la domeniul respectiv. Mai târziu există posibilitatea de a
      echivala materii comune.`,
  },
  {
    title: "Automatica de la Universitatea Politehnica București vs FMI",
    text: `Ambele facultăți te pot pregăti pentru o viitoare carieră,
...
let idx = lunr(function () {
  ...
  documents.forEach((doc) => {
    // remove diacritics for better search indexing
    doc.title = doc.title.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    doc.text = doc.text.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    doc.id = k;
    k++;
...
function searchTerm(str) {
  ...
  let results = idx.search(str);

  results = results.map((result) => result.ref);

  console.log(results.map((result) => documents[result]));
}
